...
// GET /api/segments?collection=<name>&page=...
export async function GET(req: NextRequest) {
    try {
        const db = await getDb();
        const { searchParams } = req.nextUrl;

        const collectionName = searchParams.get('collection') || DEFAULT_COLLECTION;
        ...
        return NextResponse.json({
            ...
            total_pages,
            current_page,
            next: current_page < total_pages ? `/api/segments?collection=${collectionName}&page=${current_page + 1}` : null,
            previous: current_page > 1 ? `/api/segments?collection=${collectionName}&page=${current_page - 1}` : null,
            results,
        });
    } catch (err: any) {
